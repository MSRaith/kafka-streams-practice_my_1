...
import static ru.netology.dsw.processor.SUMAlertsApp.STATE_STORE_NAME;
...
public class SUMAlertTrasformer implements Processor<String, GenericRecord, String, GenericRecord> {
    ...
    public void process(Record<String, GenericRecord> record) {
        ...
        Double quant_dbl = Double.valueOf(quantity);
        ...
        Double newVal = oldVal != null ? oldVal + price * quant_dbl : price * quant_dbl;
    ...
    private void sendAlerts(long timestamp) {
        ...
        stateStore.range(createKey(lastProcessedWindowEnd, ""), createKey(nearestMinutesTs, ""))
                        .forEachRemaining(keyVal -> {
                            ...
                            Double count = keyVal.value;
                            if (count > SUMAlertsApp.MAX_PURCHASES_PER_MINUTE) {
                                ...
                                Schema schema = SchemaBuilder.record("SUMAlert").fields()
                                        ...
                                        .noDefault()
                                        .requiredDouble("sum_price_of_purchases")
                                        .endRecord();
                                GenericRecord record = new GenericData.Record(schema);
                                // старт окна у нас в миллисекундах
                                record.put("window_start", ts);
                                record.put("sum_price_of_purchases", count);
